...
def calculate_label_frequency(
    train_dataset, valid_dataset, test_dataset, path, multilabel=False, title=None
):
...
def create_label_frequency_boxplot(dataset, path, multilabel=False, title=None):
    ...
    all_labels = [
        tuple(label) if isinstance(label, np.ndarray) else label for label in all_labels
    ]
    ...
    outliers = all_labels[
        (all_labels["Frequency"] < lower_bound)
        | (all_labels["Frequency"] > upper_bound)
    ]
    ...
    fig.add_trace(
        go.Box(
            ...
            jitter=0.3,
            pointpos=-1.8,
            marker=dict(color="blue", size=4, line=dict(color="darkblue", width=2)),
            line=dict(color="darkblue"),
            hoverinfo="text",
            hovertext=[
                f"Label: {label}<br>Frequency: {freq}"
                for label, freq in label_distribution.itertuples()
            ],
        )
    )
    ...
    fig.add_trace(
        go.Scatter(
            ...
            hovertext=[
                f"Label: {label}<br>Frequency: {freq}"
                for label, freq in outliers.itertuples()
            ],
        )
    )
...
def display_class_specific_images(
    dataset, name, path, class_types, images_per_class=5, font_size=10
):
    # Set up the plot
    rows = len(class_types)
    cols = images_per_class
    fig, axes = plt.subplots(
        rows, cols, figsize=(5 * cols, 5.5 * rows)
    )  # Increased height for padding

    for row, class_type in enumerate(class_types):
        # Find all indices for the current class
        class_indices = [
            i for i, example in enumerate(dataset) if example["answer"] == class_type
        ]
